...
#  define MCP3x6x_SPI_ADR (0b01000000)  //!< DEVICE ADDRESS
#endif
...
enum __attribute__((packed)) mux {
  ...
  Mux_TEMP_M  = 14,  //!< Internal Temperature Sensor Diode M (Temp Diode M)
  Mux_TEMP_P  = 13,  //!< Internal Temperature Sensor Diode P (Temp Diode P)
  MUX_REFIN_M = 12,  //!< REFIN-
  MUX_REFIN_P = 11,  //!< REFIN+
  MUX_AVDD    = 9,   //!< AVDD
  MUX_AGND    = 8,   //!< AGND
  MUX_CH7     = 7,   //!< CH7
  MUX_CH6     = 6,   //!< CH6
...
union Config2 {
  ...
  struct {
    uint8_t reserved : 2;  //!< reserved // Should always be equal to ‘11’
...
union Irq {
  ...
  struct {
    ...
    bool unimplemented = false;  //!< unimplemented
  };
  uint8_t raw;  //!< raw access to register
};
...
/**
 * @brief base class MCP
 *
 */
class MCP {
 private:
  status_t _status;
  Adcdata _adcdata = {.channelid = MCP3x6x_CH0, .value = 0};

  uint8_t _pinCS;
  SPIClass *_spi;
  SPISettings _spiSettings;
  uint8_t _pinMISO, _pinMOSI, _pinCLK;
  ...
  const size_t _RESOLUTION_MAX, _CHANNELS_MAX;
 ...
 protected:
  Config0 _config0 = MCP3x6x_CFG_CONFIG0;
  Config1 _config1 = MCP3x6x_CFG_CONFIG1;
  Config2 _config2 = MCP3x6x_CFG_CONFIG2;
  Config3 _config3 = MCP3x6x_CFG_CONFIG3;
  Irq _irq         = MCP3x6x_CFG_IRQ;
  Mux _mux         = MCP3x6x_CFG_MUX;
  Scan _scan       = MCP3x6x_CFG_SCAN;
  Timer _timer     = MCP3x6x_CFG_TIMER;
  Offset _offset   = MCP3x6x_CFG_OFFSET;
  Gain _gain       = MCP3x6x_CFG_GAIN;
  Lock _lock       = MCP3x6x_CFG_LOCK;
  Crccfg _crccfg   = MCP3x6x_CFG_CRCCFG;
 ...
 public:
   ...
   * @param pinCLK
   */
  MCP(size_t resolution, size_t channels, uint8_t pinCS = SS, SPIClass *theSPI = &SPI,
      SPISettings theSPISettings = SPISettings(), uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO,
      uint8_t pinCLK = SCK);
  ...
  bool begin(conv_mode conv = ONESHOT_SHUTDOWN, adc_mode adc = SHUTDOWN,
             data_format format = SGN_DATA);
  ...
  void configure(Config0 config0 = MCP3x6x_CFG_CONFIG0, Config1 config1 = MCP3x6x_CFG_CONFIG1,
                 Config2 config2 = MCP3x6x_CFG_CONFIG2, Config3 config3 = MCP3x6x_CFG_CONFIG3,
                 Irq irq = MCP3x6x_CFG_IRQ, Mux mux = MCP3x6x_CFG_MUX, Scan scan = MCP3x6x_CFG_SCAN,
                 Timer timer = MCP3x6x_CFG_TIMER, Offset offset = MCP3x6x_CFG_OFFSET,
                 Gain gain = MCP3x6x_CFG_GAIN, Lock lock = MCP3x6x_CFG_LOCK,
                 Crccfg crccfg = MCP3x6x_CFG_CRCCFG);
   ...
   * @param callback
   */
  void attachIRQ(uint8_t _pinIRQ, void (*callback)(void), Irq irq = MCP3x6x_CFG_IRQ);
   ...
   * @param _pinMCLK
   */
  void attachMCLK(uint8_t _pinMCLK, enum clk_sel clk = EXTERN);
   ...
   * @param lock
   */
  void lock(uint8_t lock = MCP3x6x_CFG_LOCK);
  ...
  inline status_t write(Config0 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_CONFIG0);
  }
  ...
  inline status_t write(Config1 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_CONFIG1);
  }
  ...
  inline status_t write(Config2 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_CONFIG2);
  }
  ...
  inline status_t write(Config3 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_CONFIG3);
  }
  ...
  inline status_t write(Irq data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_IRQ);
  }
  ...
  inline status_t write(Mux data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IWRITE | MCP3x6x_ADR_MUX);
  }
  ...
  inline status_t read(Config1 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IREAD | MCP3x6x_ADR_CONFIG1);
  }
  ...
  inline status_t read(Config2 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IREAD | MCP3x6x_ADR_CONFIG2);
  }
  ...
  inline status_t read(Config3 data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IREAD | MCP3x6x_ADR_CONFIG3);
  }
  ...
  inline status_t read(Irq data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IREAD | MCP3x6x_ADR_IRQ);
  }
  ...
  inline status_t read(Mux data) {
    return _transfer16(data.raw, MCP3x6x_CMD_IREAD | MCP3x6x_ADR_MUX);
  }
  ...
  /**
   * @brief get resolution to specific amount of bits
   *
   * @param bits
   */
   ...
   * @param sel
   */
  void setReference(float vref = 2.4, bool sel = true);
   ...
   * @return uint32_t
   */
  uint8_t getLatestChannel() { return _adcdata.channelid; }
   ...
   */
  void calibrate();
  void enableGain(enum gain digital, const uint8_t *analog = MCP3x6x_CFG_GAIN);
  void enableOffset(const uint8_t *off = MCP3x6x_CFG_OFFSET);
};
};  // namespace MCP3x6x
 ...
 *
 */
class MCP3461 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3461(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(16, 1, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
 ...
 *
 */
class MCP3462 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3462(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(16, 2, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
 ...
 *
 */
class MCP3464 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3464(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(16, 4, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
 ...
 *
 */
class MCP3561 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3561(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(24, 1, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
 ...
 *
 */
class MCP3562 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3562(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(24, 2, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
 ...
 *
 */
class MCP3564 : public MCP3x6x::MCP {
 public:
  /**
   ...
   * @param pinCLK
   */
  MCP3564(uint8_t pinCS = SS, SPIClass *theSPI = &SPI, SPISettings theSPISettings = SPISettings(),
          uint8_t pinMOSI = MOSI, uint8_t pinMISO = MISO, uint8_t pinCLK = SCK)
      : MCP(24, 4, pinCS, theSPI, theSPISettings, pinMOSI, pinMISO, pinCLK) {}
};
